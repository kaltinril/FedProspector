...
- [x] Try shelling out to `7z` (7-Zip) first — multi-threaded, significantly faster on large archives
- [x] Fall back to Python `zipfile` if 7z is not on PATH
- [x] Log which method was used and extraction time

### Task 2: Fast CSV parsing

- [x] Switch from `csv.DictReader` to `csv.reader` with a header-to-index mapping
- [x] Build column index map from the header row once, then access by index instead of key
- [x] This avoids creating a dict for every row (millions of rows)
- [x] Expected improvement: 2-3x faster CSV reading

### Task 3: MySQL bulk load session optimizations

- [x] At the start of a bulk load session, run these SET statements (session-scoped, not global):
  ```sql
  ...
  ```
- [x] Restore defaults at end of load (or just let session close)
- [x] Log that bulk optimizations are active
- [x] Note: `sql_log_bin` requires SUPER or SESSION_VARIABLES_ADMIN privilege. If fed_app doesn't have it, skip gracefully and log a warning.

### Task 4: Reduce binary log / undo log file bloat

- [x] Document in this file: users can purge old binlogs with `PURGE BINARY LOGS BEFORE NOW() - INTERVAL 1 DAY`
- [x] Document: binary logging can be disabled entirely for dev/single-server setups by adding `skip-log-bin` to my.ini
- [x] Add `skip-log-bin` to the reference my.ini at thesolution/reference/mysql-my.ini with a comment explaining when it's safe (single server, no replication)
- [x] Document: `innodb_undo_log_truncate = ON` and `innodb_max_undo_log_size` can limit undo tablespace growth
