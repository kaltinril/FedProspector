...
- [ ] **Task 7:** Thread-safe in-batch dedup tracking (`seen_hashes` dict with `threading.Lock`) in both downloader and extractor *(extractor half done)*
- [ ] **Task 8:** Add dedup stats section to `pipeline-status` CLI command
- [ ] **Task 9:** Add logging so daily loads report dedup skips (count, canonical references, method) *(extractor half done)*
