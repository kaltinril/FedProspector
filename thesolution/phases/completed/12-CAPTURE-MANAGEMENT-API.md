...
**Total endpoints this phase: 14 (11 write + 3 read)**
...
- [x] Validate: `noticeId` exists in opportunity table, not already tracked
- [x] Set initial status = `NEW`, auto-assign if user specified
- [x] Auto-calculate initial Go/No-Go score
- [x] Auto-create `STATUS_CHANGE` note: "Prospect created with status NEW, priority {priority}."
- [x] Log activity: "Created prospect from opportunity {noticeId}"
...
- [x] Filters: `status`, `assignedTo`, `captureManagerId`, `priority`, `naics`, `setAside`, `openOnly` (exclude terminal statuses)
- [x] Sort by: `responseDeadline`, `estimatedValue`, `goNoGoScore`, `createdAt`
- [x] Pagination standard
- [x] JOIN to opportunity for title, deadline, set-aside, NAICS, department, active flag
- [x] JOIN to app_user for assigned username/display_name
- [x] `openOnly=true` excludes statuses: WON, LOST, DECLINED, NO_BID

#### `GET /api/v1/prospects/{id}` -- Full prospect detail
- [x] Full prospect record with all columns
- [x] Nested: `notes[]`, `teamMembers[]`, `proposal` (if exists), `opportunity` detail
- [x] Include current Go/No-Go score breakdown
- [x] Notes ordered by created_at ASC
- [x] Team members joined to entity table for legal_business_name
...
- [x] **CRITICAL**: Replicate exact status flow from Python ProspectManager
...
- [x] Validate new assignee exists and is active
- [x] Auto-create `ASSIGNMENT` note: "Reassigned from {oldUsername} to {newUsername}."
- [x] Log activity
...
- [x] Validate note_type against allowed list: `COMMENT`, `ASSIGNMENT`, `DECISION`, `REVIEW`, `MEETING`
- [x] `STATUS_CHANGE` notes are auto-generated by the system during status transitions and cannot be created manually.
- [x] (Extended types also accepted for API flexibility: `PHONE_CALL`, `EMAIL`)
- [x] Link to current authenticated user
- [x] Validate prospect exists; return 404 if not found
- [x] Log activity
...
- [x] `ueiSam` links to entity table; warn (but allow) if UEI not found
- [x] Role must be one of: `PRIME`, `SUB`, `MENTOR`, `JV_PARTNER`
- [x] Validate prospect exists; return 404 if not found
- [x] Log activity
...
- [x] Hard delete (team member records are lightweight, no history needed)
- [x] Validate prospect and member exist; return 404 if not found
- [x] Log activity
...
- [x] **CRITICAL**: Replicate exact scoring from Python ProspectManager `calculate_score()` (lines 899-1048)
...
#### Scoring endpoints
- [x] Auto-recalculate on prospect create and status change
- [x] Score stored in `prospect.go_no_go_score` column
- [x] Expose score breakdown in prospect detail endpoint (see 12.1 response example)
- [x] `POST /api/v1/prospects/{id}/recalculate-score` -- manual recalculation trigger
...
- [x] Enforce 1:1 relationship (UNIQUE KEY on prospect_id)
- [x] Set initial status = DRAFT
- [x] Auto-create default milestones (configurable list):
  - Draft Due
  - Internal Review
  - Final Submission
  - Q&A Period
...
- [x] Update status, pricing, dates, document count
- [x] Proposal status flow:
  ```
...
- [x] Sync `proposal_status` to `prospect.proposal_status` for quick filtering
- [x] On SUBMITTED: auto-set `submitted_date = NOW()`
- [x] On AWARDED: auto-transition prospect status to WON (using StatusFlow validation)
- [x] On NOT_AWARDED: auto-transition prospect status to LOST (using StatusFlow validation)
...
- [x] Create `proposal_document` record
- [x] Increment `proposal.document_count`
- [x] File storage handled by C# API (filesystem or blob storage -- not in MySQL)
- [x] MySQL stores metadata only: filename, content type, size, upload timestamp
...
- [x] All milestones for this proposal
- [x] Sort by planned_date ASC

#### `PATCH /api/v1/proposals/{id}/milestones/{milestoneId}` -- Update milestone
- [x] Update actual_date, status, notes
- [x] Log activity
...
- [x] All write operations create `activity_log` records automatically
- [x] Implement as a service called from each controller action (or middleware)
- [x] Action types:
  - `CREATE_PROSPECT`
  - `UPDATE_STATUS`
  - `REASSIGN_PROSPECT`
  ...
  - `UPDATE_MILESTONE`
  - `RECALCULATE_SCORE`
- [x] Include `old_value` / `new_value` for status changes (JSON strings)
- [x] Include IP address from `HttpContext.Connection.RemoteIpAddress`
- [x] Include authenticated user ID from JWT claims

---

## Acceptance Criteria

1. [x] Prospect CRUD: create, list, detail, status transition all work
2. [x] Status flow enforced: invalid transitions return 400 with explanation message matching Python format
3. [x] Terminal statuses (WON, LOST, DECLINED, NO_BID) set outcome, outcome_date, outcome_notes automatically
4. [x] BID_SUBMITTED status sets bid_submitted_date automatically
5. [x] Go/No-Go scoring matches Python output for same opportunity data (all 4 criteria, 0-40 scale)
6. [x] Score breakdown visible in prospect detail response
7. [x] NAICS match scoring queries entity_naics + entity_business_type with codes 2X, 8W, A2
8. [x] Proposal lifecycle: DRAFT -> IN_REVIEW -> SUBMITTED -> UNDER_EVALUATION -> AWARDED/NOT_AWARDED works
9. [x] Proposal-Prospect sync: AWARDED auto-transitions prospect to WON, NOT_AWARDED to LOST
10. [x] Document upload creates metadata record and increments document_count
11. [x] Milestones auto-created on proposal creation, individually updateable
12. [x] Activity log records created for all write operations with old/new values
13. [x] All endpoints require authentication (JWT Bearer)
14. [x] Swagger shows request/response examples for all endpoints
15. [x] Note types validated against allowed list (COMMENT, ASSIGNMENT, DECISION, REVIEW, MEETING); STATUS_CHANGE is system-only
16. [x] Team member roles validated against allowed list (PRIME, SUB, MENTOR, JV_PARTNER)
17. [x] Priority validated against allowed list (LOW, MEDIUM, HIGH, CRITICAL)
