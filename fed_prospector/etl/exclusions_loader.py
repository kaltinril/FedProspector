...
class ExclusionsLoader(StagingMixin):
    ...
    # Pre-computed upsert SQL for use with unique index on (uei, activation_date, exclusion_type)
    _UPSERT_SQL = build_upsert_sql(
        table="sam_exclusion",
        columns=_UPSERT_COLS,
        pk_fields={"uei", "activation_date", "exclusion_type"},
        timestamp_cols={
            "first_loaded_at": "NOW()",
            "last_updated_at": "NOW()",
        },
    ...
    def load_exclusions(self, exclusions_data, load_id):
        ...
        try:
            for batch_start in range(0, len(exclusions_data), self.BATCH_SIZE):
                batch_raw = exclusions_data[batch_start : batch_start + self.BATCH_SIZE]
                    ...
                    try:
                        ...
                        if old_hash and old_hash == new_hash:
                            ...
                            changed_records.append(
                                (raw, exclusion_data, record_key, new_hash)
                            )

                        normalized.append((raw, exclusion_data, record_key, new_hash))
                ...
                if changed_records:
                    try:
                        upsert_rows = []
                        for _raw, exclusion_data, _rk, _nh in changed_records:
                            values = [
                                exclusion_data.get(c) if c != "last_load_id" else load_id
                        ...
                        for _raw, exclusion_data, record_key, new_hash in changed_records:
                        ...
                        conn.rollback()
                        for raw, exclusion_data, record_key, new_hash in changed_records:
                            try:
